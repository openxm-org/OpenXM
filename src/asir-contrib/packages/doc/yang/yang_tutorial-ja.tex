...
されました($\theta_i = x_i\partial_i$).  環の元を定義しましょう.
...
演算子の内部表現は分散多項式になっていますので, そのままでは
$R'=K[x_1,x_2]\langle \theta_1, \theta_2\rangle$ の元しか表せず, $R$ の
...
$L_1 = S_1 (S + c-1) - x_1 (S_1 + b_1)(S+a)$ です.  $R'$ における演算子
の積は \verb|yang.multi| で計算します.  ただし, $K[x_1,x_2]$ の元はそのま
まかけても構いません.  いまのところ, \verb|yang.multi| の引数に使えるの
は$R'$ の元のみです.
...
$R$ のイデアル $I=\{ L_1, L_2 \}$ のグレブナ基底 $G$ を計算します.
$I$ は \verb|[[L1,1],[L2,1]]| のように表すこともできます.  計算結果は
\begin{eqnarray*}
G = &\biggl\{
&
