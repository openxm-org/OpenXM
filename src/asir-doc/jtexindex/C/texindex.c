...
init_char_order (void)
    ...
    /* 以下のシンボルは無視 (i.e. char_order[XXX] = 0) */
    for (i = 0x2121; i <= 0x2132; i++) /* 全角空白 〜 ＿ */
        char_order[i] = 0;
    for (i = 0x213D; i <= 0x215B; i++) /* − 〜 】 */
        char_order[i] = 0;

    /*	促音、拗音、濁音、半濁音、片仮名/平仮名の処理	*/
    /*	    1.  平仮名と片仮名、同じに扱う  */
    /*		ぁ/ァ(2421/2521)〜ん/ン(2473/2573)  */
    ...
    /*	    3.	特殊片仮名の扱い    */
    char_order[0x2574]	= 0x2426;	/* ヴ	→  う */
    char_order[0x2575]	= 0x242B;	/* ヵ	→  か */
    char_order[0x2576]	= 0x2431;	/* ヶ	→  け */

    /*	JISX0208のアルファベット、数字  */
    for (i = 0x2330; i <= 0x2339; i++) /* ０〜９ -> 0-9 */
        char_order[i] = i - 0x2330 + 512;
    for (i = 0x2341; i <= 0x235A; i++) /* Ａ〜Ｚ、ａ〜ｚ -> A-Z */
     ...
     long pos2;
{
  ...
  if (keyfield->numeric)
    ...
    /* lexical ってか? */
	  ...
	  /* char_order[X] が 0 のもの（シンボル）は無視する */
	  c1 = c2 = 0;
	  while (p1 < e1)
	    {
...
wfwrite(u_short *ptr, size_t  size,  size_t  nmemb,  FILE*stream)
    ...
    cptr    = (char *)xmalloc(size * nmemb * sizeof(u_short)); /* 丼一杯! */
...
Vowel_of(int c)
    {
    if (0x2521 <= c && c < 0x2577)		/* 片仮名は */
	c   -= 0x100;				/* 平仮名へ */
    /* ヴ、ヵ、ヶ、の3字は相当する平仮名はないが、コードがあると仮定する */
    /* 普通は問題にならない筈。	*/
...
init_Vowel_Table(void)
	...
	{
	Vowel_Table[i]	    = Vowel_Table[i+1]    = 0x2422; /* あ段 */
	Vowel_Table[i+2]    = Vowel_Table[i+3]    = 0x2424; /* い段 */
	Vowel_Table[i+4]    = Vowel_Table[i+5]    = 0x2426; /* う段 */
	Vowel_Table[i+6]    = Vowel_Table[i+7]    = 0x2428; /* え段 */
	Vowel_Table[i+8]    = Vowel_Table[i+9]    = 0x242A; /* お段 */
	}
    Vowel_Table[0x243F - 0x2420]    = 0x2422;	/* た */
    Vowel_Table[0x2440 - 0x2420]    = 0x2422;	/* だ */
    Vowel_Table[0x2441 - 0x2420]    = 0x2424;	/* ち */
    Vowel_Table[0x2442 - 0x2420]    = 0x2424;	/* ぢ */
    Vowel_Table[0x2443 - 0x2420]    = 0x2426;	/* っ */
    Vowel_Table[0x2444 - 0x2420]    = 0x2426;	/* つ */
    Vowel_Table[0x2445 - 0x2420]    = 0x2426;	/* づ */
    Vowel_Table[0x2446 - 0x2420]    = 0x2428;	/* て */
    Vowel_Table[0x2447 - 0x2420]    = 0x2428;	/* で */
    Vowel_Table[0x2448 - 0x2420]    = 0x242A;	/* と */
    Vowel_Table[0x2449 - 0x2420]    = 0x242A;	/* ど */
    Vowel_Table[0x244A - 0x2420]    = 0x2422;	/* な */
    Vowel_Table[0x244B - 0x2420]    = 0x2424;	/* に */
    Vowel_Table[0x244C - 0x2420]    = 0x2426;	/* ぬ */
    Vowel_Table[0x244D - 0x2420]    = 0x2428;	/* ね */
    Vowel_Table[0x244E - 0x2420]    = 0x242A;	/* の */
    /* は〜ぽ */
    ...
    Vowel_Table[0x245E - 0x2420]    = 0x2422;	/* ま */
    Vowel_Table[0x245F - 0x2420]    = 0x2424;	/* み */
    Vowel_Table[0x2460 - 0x2420]    = 0x2426;	/* む */
    Vowel_Table[0x2461 - 0x2420]    = 0x2428;	/* め */
    Vowel_Table[0x2462 - 0x2420]    = 0x242A;	/* も */
    Vowel_Table[0x2463 - 0x2420]    = 0x2422;	/* ゃ */
    Vowel_Table[0x2464 - 0x2420]    = 0x2422;	/* や */
    Vowel_Table[0x2465 - 0x2420]    = 0x2426;	/* ゅ */
    Vowel_Table[0x2466 - 0x2420]    = 0x2426;	/* ゆ */
    Vowel_Table[0x2467 - 0x2420]    = 0x242A;	/* ょ */
    Vowel_Table[0x2468 - 0x2420]    = 0x242A;	/* よ */
    Vowel_Table[0x2469 - 0x2420]    = 0x2422;	/* ら */
    Vowel_Table[0x246A - 0x2420]    = 0x2424;	/* り */
    Vowel_Table[0x246B - 0x2420]    = 0x2426;	/* る */
    Vowel_Table[0x246C - 0x2420]    = 0x2428;	/* れ */
    Vowel_Table[0x246D - 0x2420]    = 0x242A;	/* ろ */
    Vowel_Table[0x246E - 0x2420]    = 0x2422;	/* ゎ */
    Vowel_Table[0x246F - 0x2420]    = 0x2422;	/* わ */
    /* ゐ〜ヶ は、そのまま。母音への変換はしない。 */
