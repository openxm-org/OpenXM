...
Dresolution2 Module := options -> M -> (
     ...
     pInfo (2, "ä‹t Degree " | 0 | "...");
     pInfo (2, "ä‹tä‹tä‹t Rank = " | rank target N | "ä‹t time = 0. seconds");
     pInfo (2, "ä‹t Degree " | 1 | "...");
     tInfo := toString first timing (m := schreyerOrder gens gb N);
     pInfo (2, "ä‹tä‹tä‹t Rank = " | rank source N | "ä‹t time = " |
	  tInfo | " seconds");
...
Dresolution2 (Module, List) := options -> (M, w) -> (
     ...
     pInfo (2, "ä‹t Degree 0...");
     pInfo (2, "ä‹tä‹tä‹t Rank = " | t | "ä‹t time = 0 seconds");
     pInfo (3, "ä‹t Degree 1...");
     ...
     pInfo(2, "ä‹tä‹tä‹t Rank = " | #shiftvec | "ä‹t time = " |
	  tInfo | " seconds");
     startDeg := 2;
...
-- NO INTEGER ROOTS
     if #intRoots == 0 then (
     	  k0 := 0;
     	  k1 := 0;
     	  pInfo(4, "ä‹t bFunction = " | toString b);
     	  pInfo(2, "ä‹t No integer roots");
     	  pInfo(3, "ä‹t time = " | tInfo); 
	  if member(RestrictComplex, output) then (
...
-- INTEGER ROOTS EXIST
     ...
     k1 = max intRoots;
     pInfo(4, "ä‹t bFunction = " | toString b);
     pInfo(2, "ä‹t min root =  " | k0+1 | " , max root =  " | k1);
     pInfo(3, "ä‹t time = " | tInfo | " seconds");
     pInfo(2, " ");
...
-- COMPUTE FREE RESOLUTION ADAPTED TO THE WEIGHT VECTOR "w"
     ...
     pInfo(2, "ä‹t Finished...");
     pInfo(2, "ä‹tä‹tä‹t Total time = " | tInfo | " seconds");
     pInfo(2, " ");
...
-- COMPUTE THE RESTRICTED COMPLEX IN DEGREES "n0" TO "n1" 
     ...
     tInfo = toString first timing (
	  ...
	  -- MAKE THE SOURCE MODULE AS DIRECT SUM OF D_m MODULES
	       ...
	       -- They have the form { ä‹prod_{i=1}^n D_i^{a_i} }.
	  ...
	  -- MAKE THE DIFFERENTIAL AS MATRIX OF D_m MODULES
	       ...
	       -- For each generator j = ä‹prod_i D_i^{a_i}, compute its image
     ...
     pInfo(2, "ä‹tä‹tä‹t Rank = " | sourceSize | "ä‹t time = " | tInfo | " seconds" );
     ...
     while currDeg <= n1 and C#?(currDeg) do (
	  ...
	  currDeg = currDeg+1;
     	  ...
     	  pInfo(2, "ä‹tä‹tä‹t Rank = " | targetSize | "ä‹t time = " | tInfo | " seconds");
     	  );
     );
     pInfo(2, "ä‹t Finished...");
     pInfo(2, "ä‹tä‹tä‹t Total time = " | tInfo | " seconds");
