...
static const uint32_t __blowfish_tbl_s[] = {
...
arctan(t) = t - t^3/3 + t^5/t + ... + (-1)^{2n+1} (t^{2n+1}) / (2n+1) + ...
...
/*
 * N 8000, LEN 1000000
 *  2629.921u 56.968s 46:11.96 96.9%        145+3293k 0+0io 0pf+0w
 */
...
main()
{
	...
	for (i = 0; i < M; i++) {
		...
		printf("0x%02x%02x%02x%02xU,%s", p[3], p[2], p[1], p[0],
			(i % 4 == j) ? "\n" : " ");
		if (i == 17) {
			printf("\n");
			j = 1;
