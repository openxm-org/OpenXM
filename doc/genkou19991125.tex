...
%% $OpenXM: OpenXM/doc/genkou19991125.tex,v 1.114 1999/12/28 14:53:40 tam Exp $
...
数学的なデータの表現とバイト列への変換方法を提供するプロジェクト.
このプロジェクトでは, 数学ソフト間で数学データをやりとりする際の
バイナリ表現を規定している.
MP は数式を構文木の一種で表しており, OpenMath と同様,
異なる種類の数式処理システム間で異なるオブジェクトに取り違えられないよう,
情報を付加できるようになっている.
また, 変換されたデータはデータ長が小さくなるよう考慮されており,
...
ピアツーピアのストリームコネクションを
行なう.
交換に用いられる数学データの表現方法についての規定はない. したがって数学
的なデータの表現には MP や OpenXM で定められたものを利用することができる.
