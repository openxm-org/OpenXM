...
%% $OpenXM: OpenXM/doc/genkou19991125.tex,v 1.98 1999/12/26 07:42:22 tam Exp $
...
OpenXM 規約では通信の方法に自由度があるが, 現在のところは TCP/IP 
を用いた通信しか実装されていない.  \footnote{asir には MPI を用いた実装
もある.}  そこで, この論文では TCP/IP を用いた実装に準拠して
OpenXM の説明をする.
...
バとクライアントの間で行なわれる.
\footnote{現在, 主に野呂が OpenXM の計算モデルの拡張を考えている.
効率的分散計算のアルゴリズムの多くはサーバ同士の通信も要求する.}
クライアントからサーバへメッセージを送
...
後述する OpenMath ではこの文章を phrase dictionary と呼んでいる.
...
それぞれ数学システムの名前, バージョン, HOSTTYPE を表すことになっている.
\begin{quote}
(CMO\_LIST, {\sl int32},
...
\section{OpenXM 以外のプロジェクト}

OpenXM 以外にも数式処理システム間の通信を目指したプロジェクトは存在する. 
...
現を利用することができる.  実際の情報交換の手続きはこのプロジェクトの
範囲外であるが, 別のプロジェクトとして,
例えば MCP を用いた実装があり, GAP と Axiom の間で通信が行わ
れている.  OpenXM は OpenMath 規約の phrasedictionary と同じアイデアを用
いている.
...
ことができるようにするのが目標である.  現在すでに, C 言語で利用可能なラ
イブラリが提供されている.
...
バモデルを採用しており, ピアツーピアのストリームコネクションを行なう. 
数学的なオブジェクトをどのように表現するかについては規定されておらず,
MP や MathML で定められた方法で表現することが考えられる.
すでに OpenMath の XML 表現を用いた実装が存在する.
この場合 MCP によって送信されるデータは, 本文に OpenMath 形式で数式を記
述したテキストである.
