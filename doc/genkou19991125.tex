...
%% $OpenXM: OpenXM/doc/genkou19991125.tex,v 1.87 1999/12/25 12:10:39 tam Exp $
...
この場合に用いられる整数の表現方法について説明しなければならない.
32 ビットの正の整数
$d_0 + d_1 \cdot 2^{32} + d_2 \cdot (2^{32})^2 + d_2 \cdot (2^{32})^3$
($0 <= d_i < 2^{32}$ )
をバイト列で表す場合,
...
このプロジェクトは数学的なオブジェクトをコンピュータ上で表現する方
法を規定している.
