...
%% $OpenXM: OpenXM/doc/genkou19991125.tex,v 1.88 1999/12/25 13:05:20 tam Exp $
...
この場合に用いられる 32 ビット整数の表現方法について説明しなければならない.
32 ビットの正整数
$d_0 + d_1 \cdot 2^8 + d_2 \cdot (2^8)^2 + d_3 \cdot (2^8)^3$
($0 \le d_i < 2^8$ )をバイト列で表す場合,
...
\begin{enumerate}
\item	まず, クライアントがサーバへオブジェクトを送る.
	サーバは送られてきたオブジェクトをスタックに積む.
\item	クライアントがサーバに計算の命令を送ると,
	サーバはあらかじめ定めれらた動作を行う.
	一部の命令はスタックの状態を変更する.
	例えば SM\_executeFunction, \\
	SM\_executeStringByLocalParser などの命令は,
	スタック上のオブジェクトから計算を行う.
...
サーバは接続が必要になった時のみ起動している.

しかし, これだけでは接続を行なう一瞬のすきを狙われる可能性もある.
そこで接続を行なう時に, 接続を行なうポート番号を毎回変えている.
こうすることで, 特定のポート番号を狙って接続を行なう手口を幾らか
防ぐことができる.

さらにもう一段安全性を高めるために, 接続時に一時パスワードを
クライアントが作成し, そのパスワードを使って認証を行なう.
このパスワードは一旦使用されれば無効にするので,
もし仮になんらかの手段でパスワードが洩れたとしても安全である.

%なお, 上記のポート番号とパスワードは安全な手段で送られていると仮定してい
...
数学的なオブジェクトをコンピュータ上で表現する方法を規定している.
