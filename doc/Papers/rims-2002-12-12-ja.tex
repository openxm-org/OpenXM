% $OpenXM: OpenXM/doc/Papers/rims-2002-12-12-ja.tex,v 1.3 2002/12/06 09:32:29 noro Exp $
...
\underline{ サーバの起動が煩雑 }
ということである.
とくに C と unix の枠組をはずれると OpenXM サーバを起動
するのがとても面倒になる. 
さらに起動のためのコードは OS 毎に分岐させる必要が生じる.
この煩雑さを避ける
...
Lineo の Embedix という名前の OS (Linux 系) を持つ.
...
%%Pic: coq-starts.jpg 
      ...
      \cite{tamura}.
\item すでに出版ずみの公式集をそのまま入力すると著作権の問題が生じる.
      fb では 19世紀, 20 世紀の超幾何関数のオリジナル論文, 本をもとに 
      自分達で公式集の編集をおこなっており, 著作権は OpenXM/fb が保持する.
\item 公式の入力にはミスがつきものである.
      fb では, 数値計算をもちいて明らかな誤りを検出する仕組みを提供している
      (われわれはこれを checker と呼んでいる).
      この仕組みでは OpenXM 版の mathematica および asir サーバ
      および前述の OpenXM online を用いている.
      実際に公式の入力を始めてみてわかったことであるが,
      checker のおかげで, 公式入力作業は楽しくかつ正確となった \cite{tamura}.
...
%
\bibitem{digital-lib}
Digital  Library of Mathematical Functions (Digital Abramowitz and Stegun)
...
Content Dictionaries and
   an Electronic Formula Book for Hypergeometric Functions,
preprint
%
\bibitem{wolfram}
Wolfram Research's Mathematical Functions, \\
{\tt http://functions.wolfram.com}
\end{thebibliography}
