...
使用してる自主開発の多倍長演算機能との性能差により、必ずしも Risa/Asir の
優位性が主張できなくなってきた。
...
$t$ のハッシュ値 $h_t$ を計算して, あるテーブルの $h_t$ の位置に,
...
トルを表しており、実際には変数の個数分の長さの配列がセットされる.
{\tt NM} は linked list 形式の, {\tt NMV} は配列形式の多項式における
単項式を表す. {\tt NDV} は, {oNMV} すなわち構造体そのものの
配列へのポインタを持つ. {\tt NDC} は係数を保持するための汎用の共用体
である. 
\begin{verbatim}
typedef union oNDC {
    int m;
...
各エントリは, {\tt RHist} のリストとして登録される. また {\tt ND\_pairs}
は S-pair を保持するための構造体であり, やはりリストである.
\section{各部の詳細}
...
より数倍から十数倍高速である. また, 問題にもよるが, {\tt nd\_f4} は
...
表 \ref{tab:janet} は, 種々のベンチマーク問題の計算時間を示す.
