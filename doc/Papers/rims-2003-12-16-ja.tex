%% $OpenXM$
%% この文書には ox_ntl, oxshell, OX RFC 102, 103 が解説されている.
%% ChangeLog には http://www.openxm.org の cvsweb でのソースファイル名および
%% そのバージョンが書かれている.
...
\title{OpenXM の新サーバ, 新プロトコル}
\author{OpenXM 開発グループ \\ (岩根, 小原, 野呂, 高山) \\
   OpenXM committers \\ openxm.org
%%\ename{OpenXM committers}%
% \thanks{個人毎の謝辞}%                      
%%    \affil{openxm.org} %\eaffil{所属のローマ字表記}%
%  \mail{著者のメールアドレス}%                
%  \and 
}
\date{}
...
などを目標に数年前からパッケージの開発, プロトコルの開発をしている. 
この小文では講演に用いたスライドを元に OpenXM の新しいサーバおよび新しい通信プロトコルを
紹介する.
...
\leftline{\fbox{この機能に関する ChangeLog}}
\noindent
OpenXM/src/ox\_ntl の下の全てのファイル (2003年12月まで).
OpenXM/src/ox\_toolkit の下の次のファイル.
...
\section{{\color{blue} 新しいサーバ polymake と oxshell}}
...
\subsection{ Polymake の OX 100, 101 サーバ化で何が問題であったか?}
...
unix の shell 
から利用可能なように作られている: 
このようなアプリケーションを {\color{red} バッチ処理対応アプリケーション}
と呼ぶ.
サーバとの通信の頻度は問題によるがある程度大きい計算の場合は通信時間は無視できる.
Polymake は対話的利用は想定しておらずサーバの計算を中断して, 再度開始するなどの必要がない.
したがって {\tt system} 関数
で polymake を呼び出して, 結果が書き出されたファイルを解析すればよい.
しかしこの {\tt system} 関数は使いにくい.
問題点はファイルシステムや OS の違いを吸収できない, プロセス管理のやりにくさ
などである.
我々は {\tt system} の強化版として {\color{blue} oxshell}
を導入する.


\subsection{ Oxshell とは }
...
\subsection{ oxshell.facets() の実装, 数学関数の実装例 }
...
\leftline{\fbox{この機能に関する ChangeLog}}
\noindent OpenXM/src/kan96xx の下の次の各ファイル.
\nnn Doc/oxshell.oxw 1.1--1.2.
\nnn trans の下のファイルすべて(2003年12月).
...
\noindent
\section{{\color{blue} OX-RFC 103 ( 100, 101 補遺)}}
...
\noindent
\leftline{\fbox{この機能に関するChangeLog}}
...
\section{{\color{blue} OX-RFC 102 -- 本格的なサーバ間通信を用いた分散計算}}
...
\subsection{server の起動, server 間通信路の開設}
...
\noindent
\subsection{server 間通信, broadcast, reduction}
...
\subsection{Asir (master) 上での API}
...
\noindent
\subsection{Asir (server) 上での API}
...
\noindent
\subsection{実行例 : 一変数多項式の積 (master 側)}
...
\subsection{実行例 : 一変数多項式の積 (server 側; collective)}
{\footnotesize
\begin{verbatim}
